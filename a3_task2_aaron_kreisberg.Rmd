...
author: "Aaron Kreisberg ESM 206 Fall 2020"
...
sw_split_name <- starwars %>% #Reading in the starwars data
  separate(col = name, #Separate the name column
           into = c("name_1", "name_2"), #Into "name_1" and "name_2"
           sep = "-") # Separated at the first dash

#Use View(sw_split_name) in the console to ensure the two columns show up
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "_" # Puts "_" between the pieces (default is underscore)
        ) 
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        ...
        remove = FALSE) # Retain the original column
...
some_dates_converted <- mdy(some_dates) # Converts to Date, stored as some_dates_converted
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>% # Use the 'lakers' data
  filter(player == "Pau Gasol", type == "hook") %>% # Filter for 'Pau Gasol' and 'hook' shot 
  mutate(Date = ymd(date)) %>% # Update to Date
  mutate(month = month(Date)) # Create a column with just the month

gasol_hook_by_month <- gasol_hookshots %>% 
  group_by(type, month) %>% 
  select(player, type, month) %>% 
  count(month)
# Above is the code to find the number of Gasol's attempted hook shots by month 
```
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger which contains
    status == "hurricane" ~ "high", # 'danger' if the storm is a hurricane
    status %in% c("tropical depression", "tropical storm") ~ "moderate")) # 'moderate' if the storm is tropical depression or tropical storm
